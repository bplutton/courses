...
import pytest
from graph import Graph, DAGNode, build_lab_graph, bfs, dfs, find_path
...
class TestDAGNode:
    """Test DAGNode dependency/ancestor behavior."""

    def test_add_dependency_and_ancestor(self):
        a = DAGNode("A")
        b = DAGNode("B")
        c = DAGNode("C")

        b.add_dependency(a)  # A -> B
        c.add_dependency(b)  # B -> C

        assert b.has_ancestor(a)
        assert c.has_ancestor(a)
        assert c.has_ancestor(b)
        assert not a.has_ancestor(b)

    def test_add_dependency_cycle_detection(self):
        a = DAGNode("A")
        b = DAGNode("B")

        b.add_dependency(a)  # A -> B
        with pytest.raises(ValueError):
            a.add_dependency(b)  # would create cycle
